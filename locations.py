...
for mission in all_missions:
    ...
    for rank in all_ranks:
        mission_rank_location_name_to_id[get_location_name_for_mission_rank(mission, rank)] = get_location_id_for_mission_rank(mission, rank)
...
def create_mission_rank_locations(world: AC3World) -> None:
    ...
    for mission in all_missions:
        for rank in included_ranks:
            location_to_add[get_location_name_for_mission_rank(mission, rank)] = get_location_id_for_mission_rank(mission, rank)
    mission_list.add_locations(location_to_add, AC3Location)
